...
# --- DEFINICIÓN DE RUTAS ---
BASE_DE_DATOS_FOLDER = os.path.join(os.getcwd(), "biblioteca_data")
...
def get_db():
    if 'db' not in g:
        ...
        try:
            ...
            for folder in [BASE_DE_DATOS_FOLDER, COPIAS_SEGURIDAD_FOLDER]:
                if not os.path.exists(folder):
                    os.makedirs(folder)
                    logging.info(f"Carpeta '{folder}' creada exitosamente.")
                else:
                    logging.info(f"Carpeta '{folder}' ya existe.")
            ...
            logging.error(f"Error en get_db: {e}", exc_info=True)
            if conn:
                conn.close()
            raise
    return g.db
